...
library Token {
    ...
    function _tx(
        ...
        internal
        view
        returns (uint96 taxAmount, uint96 netAmount, uint96 swapAmount)
    {
        if (sender.isEntitled() || recipient.isEntitled() || db.inSwap) {
            return (0, amount, 0);
        }
    ...
    function setIdentifier(
        ...
        _self.identifiers = uint16(
            value
                ? _self.identifiers | (1 << idx)
                : _self.identifiers & ~(1 << idx)
        );
    }
    ...
    function ratios(uint48 value) internal returns (bool output) {
        ...
        assembly {
            ...
            if iszero(iszero(or(and(ids, shl(14, 1)), and(ids, shl(15, 1))))) {
                ...
                if or(
                    or(iszero(lt(bt, 1001)), iszero(lt(st, 1001))),
                    iszero(lt(tt, 1001))
                ) {
                    revert(0, 0)
                }
        ...
        assembly {
            ...
            if iszero(iszero(or(and(ids, shl(14, 1)), and(ids, shl(15, 1))))) {
                ...
                for {
                    ...
                    dt := or(
                        and(dt, not(mask)),
                        and(shl(add(56, mul(i, 16)), v), mask)
                    )
                }
                sstore(self.slot, dt)
            }
    ...
    function init(Storage storage _self) internal {
        ...
        assembly {
            ...
            c := and(
                shr(
                    48,
                    7008354312673839259956172683932913198158069606142246319498950696839510228991
                ),
...
abstract contract ERC20 is Context, EIP712 {
    ...
    function decreaseAllowance(
        ...
        if (currentAllowance < requestedDecrease) {
            revert FailedDecreaseAllowance(
                spender,
                currentAllowance,
                requestedDecrease
            );
    ...
    function permit(
        ...
        unchecked {
            ...
            if (account == address(0) || account != holder)
    ...
    function _swapBack(uint96 amountToSwap) internal swapping {
        ...
        unchecked {
            ...
            if (settings.autoLiquidityShare > 0) {
                liquidityTokens =
                    (amountToSwap * settings.autoLiquidityShare) /
                    totalETHShares /
                    2;
            ...
            if (settings.burnShare > 0) {
                amountETHBurn =
                    (amountETH * settings.burnShare) /
                    totalETHShares;
            }

            if (settings.prizeShare > 0) {
                amountETHPrize =
                    (amountETH * settings.prizeShare) /
                    totalETHShares;
            }

            if (settings.marketingShare > 0) {
                amountETHMarketing =
                    (amountETH * settings.marketingShare) /
                    totalETHShares;
            }

            if (settings.developmentShare > 0) {
                amountETHDev =
                    (amountETH * settings.developmentShare) /
                    totalETHShares;
            }
            ...
            if (liquidityTokens > 0) {
                uint96 amountETHLiquidity = (amountETH *
                    settings.autoLiquidityShare) /
                    totalETHShares /
                    2;
...
contract CryptoXR is ERC20 {
    ...
    address public constant mainOwner =
        0x30b7f99664F5449629d06549fd0510086141b915;
    address public constant XRWorld =
        0x55d5a6809034a21B9677F418B994CAd4FC9d7B01;
    address public constant team = 0x3765eCC3A3466c94e3926DCc644779102F3bd74b;
    address public constant marketing =
        0x4B7fbE5581DA3C7783e185D7083a12EC77994770;
    address public constant investors =
        0x246f9ece9Be9f59B17176C059064433e70079827;
    address public constant partnership =
        0x5B6E8cB706302aF605FBDB36154ECe068662D30E;
    address public constant staking =
        0xF12338660Ff6C3B5b982e52eB72521499A44c6Db;

    constructor(address initialFeeCollector) ERC20("CryptoXR", "XRB", 18) {
        ...
        settings.buyTax = 100;
        settings.sellTax = 150;
        settings.transferTax = 0;
        settings.autoLiquidityShare = 0;
        settings.swapThresholdRatio = 9999;
    ...
    function recoverBNB() external {
        ...
        (bool sent, ) = payable(settings.feeRecipients[0]).call{
            value: amount,
            gas: settings.gas
        }("");
    ...
    function createPair() external onlyOwner {
        Storage storage data = token();
        data.PAIR = IPair(
            ISwapFactory(ROUTER.factory()).createPair(
                address(this),
                ROUTER.WETH()
            )
        );
